...
import { getBrandGuidelineById } from 'apiSdk/brand-guidelines';
...
function BrandGuidelineViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Brand Guideline Detail View
          </Text>
          {hasAccess('brand_guideline', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/brand-guidelines/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('brandalyzer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/brandalyzers/view/${data?.brandalyzer?.id}`}>
                      {data?.brandalyzer?.name}
                    </Link>
                  </Text>
                </Flex>
              )}
