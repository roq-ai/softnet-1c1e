...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<BrandalyzerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/brandalyzers` : null),
    () => getBrandalyzers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Brandalyzer',
      path: isTenantUser ? `/brandalyzers/view/${data?.[0]?.id}` : '/brandalyzers',
      entity: 'brandalyzer',
